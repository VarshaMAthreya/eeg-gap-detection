...
for subj in subjlist:
    ...
    fpath = froot + subj + '/'
    bdfs = fnmatch.filter(os.listdir(fpath), subj +'_LightSedation_GDT_NoGap*.bdf')
...
#%% Manual plots and saving 
    ...
    # plt.plot(t_full, ep_subderm, label='Subdermal electrode')
    # plt.fill_between(t_full, ep_subderm - ep_sem_subderm,
    #                  ep_subderm + ep_sem_subderm,alpha=0.5)
...
#%%% Plotting gap responses 
    ...
    # plt.plot(t, gap_subderm, label='Subdermal electrode')
    # plt.fill_between(t, gap_subderm - gap_sem_subderm,
    #                   gap_subderm + gap_sem_subderm,alpha=0.5)
    ...
    savemat(save_loc + subj + '_LightSedation_NoGap_2-20Hz.mat', mat_ids)
...
# Save location for all analyzed ITC files and figs across ages    
    # # ITC1 - 16 ms
    # freqs = np.arange(2., 14., 1.)
    # n_cycles = freqs * 0.2
    
    # power, itc = tfr_multitaper(epoch_gap, freqs, n_cycles, picks=picks,
    ...
    # x = (itc.data).mean(axis=1)    #Mean of the 14 freqs
    
    # t=epoch_gap.times
    
    # mat_ids = dict(fs=4096, t=t, itc=x,freqs=freqs, n_channels=picks, n_cycles=n_cycles)
    
    # savemat(save_loc + subj + '_ITCGap.mat', mat_ids)
    

    # print('WOOOHOOOO! Saved ' + subj)
    
    # del (epochs, epoch_gap, itc, power)
